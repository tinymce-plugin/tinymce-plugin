...
import {
  ...
  export default defineComponent( {
    ...
    props: {
      ...
      demoID:{
        type: String,
        default: 'demoReact'+new Date().getTime()+Math.round(Math.random()*10)
      }
    },
    created(){
    this.init()
    ...
    methods:{
      init(){
          setTimeout(()=>{
            let sourceCode = this.$parent.sourceCode(this.$React, this.$ReactDOM, Editor,document.getElementById(''+this.demoID))
